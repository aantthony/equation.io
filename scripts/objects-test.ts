...
const server = spawn(root+'node_modules/.bin/vite',['--port','5198','--strictPort'],{cwd:root,stdio:'ignore'});
let browser: Awaited<ReturnType<typeof chromium.launch>> | undefined;
try {
  for(let i=0;;i++) { try { if((await fetch(origin)).ok) break; } catch {} if(i>100) throw new Error('Vite did not start'); await new Promise(r=>setTimeout(r,200)); }
  browser=await chromium.launch({executablePath:process.env.CHROMIUM || undefined,args:['--enable-unsafe-swiftshader']});
  ...
  await page.addInitScript(()=>{
    ...
    proto.uniform1f=function(l,v){if(l && locations.get(l)?.includes('eqioFamilyIndex')) state.family.push(v);return uniform.call(this,l,v);};
  });
  const load=async(rows:string[])=>{errors.length=0;await page.goto('about:blank');await page.goto(origin+'/#'+rows.map(encodeURIComponent).join(';'));await page.waitForSelector('.eq-line');await page.waitForTimeout(250);assert.deepEqual(await page.locator('.eq-error').allTextContents(),[],rows.join(';'));};
  ...
  assert.deepEqual(errors,[]);console.log('PASS 3D points, triangle and arrow');
  ...
  assert.deepEqual(errors,[]);console.log('PASS family uniforms and shader draws');
  await load(['revolve([1,2])']);
  await page.waitForFunction(()=>[0,1].every(v=>(window as any).objectTest.family.includes(v)));
  assert.deepEqual(errors,[]); console.log('PASS 3D family shader uniforms');
  await load(['a_{n+1}=2a_n','y=a_3 x']);
  assert.deepEqual(errors,[]); console.log('PASS recurrence shader uniforms');
  ...
  assert.deepEqual(errors,[]);console.log('PASS background trajectories and arrow lattice');
  ...
  assert.deepEqual(errors,[]);console.log('PASS search-box certification');
