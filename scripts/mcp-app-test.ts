...
try {
  ...
  await page.route('http://127.0.0.1:5195/**', route =>
    route.fulfill({
      contentType: 'text/html',
      body: '<!doctype html><html><body style="margin:0"><iframe title="Graph" sandbox="allow-scripts allow-same-origin allow-popups" style="width:100%;height:520px;border:0"></iframe></body></html>',
    }),
  );
  ...
  await page.evaluate(
    ({ html, origin }) => {
      ...
      const state = window as any;
      state.messages = [];
      state.sendResult = (value: unknown) =>
        frame.contentWindow!.postMessage(
          { jsonrpc: '2.0', method: 'ui/notifications/tool-result', params: value },
          '*',
        );
      state.sendContext = (value: unknown) =>
        frame.contentWindow!.postMessage(
          { jsonrpc: '2.0', method: 'ui/notifications/host-context-changed', params: value },
          '*',
        );
      state.sendInput = (args: unknown, partial = false) =>
        frame.contentWindow!.postMessage(
          {
            jsonrpc: '2.0',
            method: `ui/notifications/tool-input${partial ? '-partial' : ''}`,
      ...
      window.addEventListener('message', event => {
        ...
        if (message.method === 'ui/initialize')
          response = {
            ...
            hostContext: {
              ...
              styles: {
                variables: {
                  '--color-background-secondary': '#20252b',
                  '--color-text-primary': '#ebf0f5',
        ...
        if (message.method === 'ui/notifications/initialized')
          state.sendInput({ equations: ['a = 2', 'y = a sin('] }, true);
        ...
        if (message.id !== undefined)
          frame.contentWindow!.postMessage({ jsonrpc: '2.0', id: message.id, result: response }, '*');
      });
      ...
      frame.srcdoc = html.replace(
        '<head>',
        `<head><meta http-equiv="Content-Security-Policy" content="default-src 'none'; script-src ${origin} blob: 'unsafe-inline'; style-src ${origin} 'unsafe-inline'; img-src ${origin} data:; worker-src blob:; connect-src 'none'; frame-src 'none'">`,
      );
    },
  ...
  await frame
    .waitForSelector('.eq-slider input[type=range]', { state: 'attached', timeout: 10000 })
    .catch(async error => {
      console.error({
        errors,
        messages: await page.evaluate(() => (window as any).messages),
  ...
  assert.ok(
    (await frame.locator('.eq-line').last().textContent())?.includes('sin('),
    'Partial arguments render before tool completion',
  );
  ...
  assert.ok(
    (await frame.locator('.eq-line').first().textContent())?.includes('1.5'),
    'Complete input restores saved edits before the result',
  );
  await frame.evaluate(() => ((window as any).earlyRow = document.querySelector('.eq-line')));
  assert.equal(
    await page.evaluate(
      () => (window as any).messages.filter((m: any) => m.method === 'ui/update-model-context').length,
    ),
    0,
    'Unconfirmed input is not published',
  );
  await page.evaluate(value => (window as any).sendResult(value), result);
  await frame.waitForFunction(() => document.getElementById('app-status')?.textContent === '');
  assert.ok(
    await frame.evaluate(() => (window as any).earlyRow === document.querySelector('.eq-line')),
    'Matching result does not rebuild the graph',
  );
  ...
  assert.ok(
    (await frame.locator('.eq-line').first().textContent())?.includes('1.5'),
    'Restores edits for the original tool result',
  );
  assert.equal(await frame.locator('#app-status').textContent(), '');
  assert.ok(
    await frame.locator('#app-reset').isVisible(),
    'Reset appears when restored edits differ from the original',
  );
  assert.ok(
    await frame
      .locator('#gl')
      .evaluate((canvas: HTMLCanvasElement) => canvas.width > 0 && !!canvas.getContext('webgl2')),
  );
  ...
  await page.waitForFunction(() =>
    (window as any).messages.some(
      (m: any) => m.method === 'ui/update-model-context' && m.params.structuredContent.equations[0].includes('3'),
    ),
  );
  ...
  assert.ok(
    await frame.evaluate(() => (window as any).confirmedRow === document.querySelector('.eq-line')),
    'Replayed input preserves the editor',
  );
  ...
  await page.waitForFunction(() =>
    (window as any).messages.some(
      (m: any) => m.method === 'ui/update-model-context' && m.params.structuredContent.equations[0].includes('4'),
    ),
  );
  await frame.locator('#app-reset').click();
  await frame.waitForFunction(() => document.querySelector('.eq-line')?.textContent?.includes('a = 2'));
  assert.ok(
    (await frame.locator('.eq-line').first().textContent())?.includes('a = 2'),
    'Reset restores the original tool-result equations',
  );
  assert.ok(await frame.locator('#app-reset').isHidden(), 'Reset hides once the graph matches the original');
  await page.waitForFunction(() =>
    (window as any).messages.some(
      (m: any) => m.method === 'ui/update-model-context' && m.params.structuredContent.equations[0].includes('a = 2'),
    ),
  );
  ...
  const beforeRequests = await page.evaluate(
    () => (window as any).messages.filter((m: any) => m.method === 'ui/request-display-mode').length,
  );
  ...
  await page.evaluate(() => {
    (window as any).forcedMode = 'inline';
    (window as any).sendContext({
      availableDisplayModes: ['inline', 'fullscreen'],
      styles: { variables: { '--color-text-primary': '#ddeeff' } },
    });
  });
  await frame.locator('#app-expand').click();
  await frame.waitForFunction(() => !(document.getElementById('app-expand') as HTMLButtonElement).disabled);
  assert.equal(
    await frame.locator('#app-expand').textContent(),
    'Expand',
    'Use the host-returned mode if fullscreen is declined',
  );
  assert.equal(
    await frame.locator('#panel').evaluate(el => getComputedStyle(el).backgroundColor),
    'rgb(32, 37, 43)',
    'Partial style updates preserve existing variables',
  );
  ...
  await page.evaluate(() => {
    ...
    document
      ...
      .contentWindow!.postMessage({ jsonrpc: '2.0', method: 'ui/notifications/tool-cancelled', params: {} }, '*');
  });
  await frame.waitForFunction(() => document.getElementById('app-status')?.textContent === 'Graph request cancelled.');
  await page.waitForTimeout(100);
  assert.ok(
    await frame.evaluate(() => (window as any).beforeCancelledPreview === document.querySelector('.eq-line')),
    'Cancellation discards queued partial input',
  );
  ...
  await frame.waitForFunction(
    () => document.getElementById('app-status')?.textContent === 'Could not load this graph. Ask to try again.',
  );
  ...
  await frame.waitForFunction(() => document.getElementById('app-status')?.textContent === '');
  assert.ok(
    (await frame.locator('.eq-line').first().textContent())?.includes('7'),
    'Result preserves edits made during validation',
  );
  ...
  assert.equal(
    await frame.evaluate(() => (window as any).renderFrames),
    pausedFrames,
    'Offscreen widgets stop rendering, including the timer fallback',
  );
  ...
  assert.equal(
    await frame.evaluate(() => (window as any).renderFrames),
    beforeViewport,
    'Incoming viewport remains unapplied while offscreen',
  );
  await frame.locator('.eq-slider input[type=range]').evaluate((input: HTMLInputElement) => {
    input.value = '9';
    input.dispatchEvent(new Event('input', { bubbles: true }));
  ...
  await page.evaluate(() =>
    document
      ...
      .contentWindow!.postMessage(
        { jsonrpc: '2.0', id: 'teardown-test', method: 'ui/resource-teardown', params: {} },
        '*',
      ),
  );
  await page.waitForFunction(() => (window as any).messages.some((m: any) => m.id === 'teardown-test' && m.result));
  assert.ok(
    await frame.evaluate(() => (window as any).savedWidgetState.privateContent.equations[0].includes('9')),
    'Teardown saves the last edit',
  );
  assert.equal(
    await frame.evaluate(() => (window as any).savedWidgetState.privateContent.equations[2]),
    viewportRows[2],
    'Teardown preserves an unapplied viewport in widget state',
  );
  const publishedRows = await page.evaluate(
    () =>
      (window as any).messages.filter((m: any) => m.method === 'ui/update-model-context').at(-1).params
        .structuredContent.equations,
  );
  ...
  await page.evaluate(value => (window as any).sendResult(value), sphere);
  await page.evaluate(() => {
    (window as any).sendInput({ equations: ['y=x'] }, true);
    (window as any).sendInput({ equations: ['y=x'] });
  });
  ...
  assert.equal(
    await frame.evaluate(() => (window as any).renderFrames),
    disposedFrames,
    'No rendering restarts after teardown',
  );
  assert.equal(await frame.locator('.eq-line').count(), 3, 'Late tool results are ignored');
  assert.deepEqual(errors, []);
  console.log(
    'PASS: production UI resource, cross-origin CSP, WebGL, worker tracing, early/partial tool input, cancellation, validation errors, sliders, reset, context updates, state restoration, links, fullscreen negotiation, host styles, safe areas, narrow layout, visibility pause/resume, and teardown',
  );
} finally {
  await browser.close();
}
