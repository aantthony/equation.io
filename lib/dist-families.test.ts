...
describe('one table of families, every consumer', () => {
  ...
  it.each(['exp', 'gamma', 'beta', 't', 'Gamma', 'BETA', 'T'])('%s over declared data stays a regression in every form', name => {
    const forms = [`Y ~ ${name}(a X)`, `Y ~ ${name} (X - 1) + c`, `Y ~ ${name} X`, `Y ~ ${name} x + b`, `Y ~ a ${name}(X)`];
