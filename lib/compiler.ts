...
export interface GpuGrid { glsl: string; gradGlsl?: [string, string]; params: string[] }
export type CpuPlan =
...
const residualOf = (equation: Expr): Expr => equation.kind === 'eq'
  ? { kind: 'bin', op: '-', a: equation.l, b: equation.r } : equation;
const splitWithin = (expr: Expr, what: string, verb: string): [Expr, Expr] => {
  try { return complexParts(expr, PATH_NODE_BUDGET); }
  catch (error) {
    if (error instanceof SplitTooLarge) throw new Error(`This complex ${what} is too large to ${verb} once split into real and imaginary parts — reduce the nesting or the powers.`);
    throw error;
  }
};
...
const realEquation = (expr: Expr): Expr => expr.kind === 'eq' ? { ...expr, l: real(expr.l), r: real(expr.r) } : real(expr);
const point = (source: PointSource, what: string): Expr[] => source.representation === 'complex'
  ? splitWithin(source.expr, what, what === 'path' ? 'sample' : 'evaluate') : source.coordinates.map(real);
const derivatives = (exprs: Expr[] | undefined, variable: string): Expr[] | undefined => {
  try { return exprs?.map(expr => diff(expr, variable)); } catch { return undefined; }
};

export function compileGridCpu(spec: LevelSetSpec): CpuGrid {
  const expr = real(spec.expr);
  let gradient: [Expr, Expr] | undefined;
  try { gradient = [diff(expr, 'x'), diff(expr, 'y')]; } catch { /* finite differences */ }
...
export function compileGridGpu(spec: LevelSetSpec): GpuGrid {
  ...
  try { gradGlsl = [toGLSL(sub(diff(spec.expr, 'x'))), toGLSL(sub(diff(spec.expr, 'y')))]; } catch { /* finite differences */ }
...
export function compileCpu(classified: Classified): CpuPlan {
  ...
  switch (object.kind) {
    case 'curve':
      switch (object.form) {
        case 'graph': { const equation = object.equation ? realEquation(object.equation) : graphEquation(real(object.rhs)); return { type: 'implicit2d', equation, residual: residualOf(equation), levels: object.levels && compileGridCpu(object.levels) }; }
        case 'implicit': return { type: 'implicit2d', residual: real(object.residual), equation: realEquation(object.equation ?? equationOf(object.residual)), levels: object.levels && compileGridCpu(object.levels) };
        case 'parametric': {
          const comps = point(object.source, 'path');
          const d1 = derivatives(comps, 'u'); const d2 = derivatives(d1, 'u');
          return { type: 'pcurve', dim: comps.length as 2 | 3, comps, tube: object.tube, d1, d2, d3: derivatives(d2, 'u') };
        }
      }
    case 'surface':
      if (object.form === 'parametric') return { type: 'psurface', comps: object.coordinates.map(real) as [Expr, Expr, Expr] };
      else {
        const equation = realEquation(object.equation ?? equationOf(object.residual));
        const height = equation.kind === 'eq'
          ? equation.l.kind === 'var' && equation.l.name === 'z' ? equation.r
            : equation.r.kind === 'var' && equation.r.name === 'z' ? equation.l : undefined : undefined;
        return { type: 'implicit3d', equation, residual: real(object.residual), heightmap: height && !freeVars(height).has('z') ? real(height) : undefined };
      }
    ...
    case 'complex-field':
      return object.form === 'fractal' ? { type: 'fractal2d', step: object.step, seed: object.seed, maxIter: object.maxIter }
    ...
    case 'system': {
      ...
      return { type: 'system', dim: 2, complexEquation: equation, residuals: a.map((c, k) => ({ kind: 'bin', op: '-', a: c, b: b[k] })), ...(parametric ? { parametric } : {}), ...(angular ? { angular: [...angular] } : {}), ...(coordinates ? { coordinates: [...coordinates] } : {}) };
    }
    case 'sequence':
    ...
    case 'note': return { type: 'note', expr: object.expr, variable: object.variable, constant: object.constant, ...(object.identity && { identity: true as const }) };
...
export function compileGpu(classified: Classified): GpuPlan {
  ...
  const gradient = (exprs: readonly Expr[], variable: string): [string, string, string] | undefined => {
    try { return exprs.map(e => toGLSL(sub(diff(e, variable)))) as [string, string, string]; } catch { return undefined; }
  };
  switch (object.kind) {
    case 'curve':
      if (object.form === 'parametric') break;
      return { type: 'implicit2d', params, field: scalar(object.equation ?? (object.form === 'graph' ? graphEquation(object.rhs) : object.residual)), levels: object.levels ? compileGridGpu(object.levels) : undefined };
    case 'surface':
      if (object.form === 'parametric') return { type: 'psurface', params, comps: object.coordinates.map(e => toGLSL(sub(e))) as [string, string, string], du: gradient(object.coordinates, 'u'), dv: gradient(object.coordinates, 'v') };
      else {
        let grad: [string, string, string] | undefined;
        try { grad = ['x', 'y', 'z'].map(v => toGLSL(sub(diff(object.equation ?? object.residual, v)))) as [string, string, string]; } catch { /* finite differences */ }
    ...
    case 'complex-field':
      if (object.form === 'fractal') {
        ...
        return { type: 'fractal2d', params, step: step.type === 'complex' ? step.code : `vec2(${step.code}, 0.0)`, seed: object.seed, maxIter: object.maxIter };
      }
      return { type: object.form === 'potential' ? 'complex2d' : object.form === 'domain' ? 'domain2d' : 'conformal2d', params, field: scalar(object.expr) };
    case 'sequence':
      ...
      return { type: 'bifurcation', params, field: compileTyped(sub(substVars(object.expr, { [object.variable]: { kind: 'var', name: 'a' } }))).code };
    case 'family':
      if (object.shared) {
...
export function shaderKey(plan: GpuPlan): string {
  switch (plan.type) {
    ...
    case 'family': return `family:${plan.members.map(shaderKey).join('|')}`;
...
export function cpuStructureKey(plan: CpuPlan): string {
  ...
  switch (plan.type) {
    ...
    case 'pcurve': structure = [expressions(plan.comps), plan.tube && exprKey(plan.tube)]; break;
    case 'vlist': structure = expressions(plan.values); break;
    case 'plist': structure = [plan.dim, plan.pts.map(expressions)]; break;
    case 'dlist': structure = plan.values.length; break;
    case 'dscatter': structure = [plan.dim, plan.coords.map(c => c.length)]; break;
    case 'histogram': structure = [plan.centers.length, plan.counts.length]; break;
