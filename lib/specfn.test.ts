...
describe('lgamma', () => {
  it('matches reference values from tiny to huge arguments', () => {
    const refs: Array<[number, number]> = [
      ...
      [1e4, 82099.71749644238],
      [1e7, 151180949.36947393],
    ];
...
describe('gammaPQ', () => {
  const refs: Array<[number, number, number, number]> = [
    ...
    [0.01, 1e-5, 0.8963367982671969, 0.10366320173280281],
    ...
    [10, 30, 0.9999928782491372, 7.12175086281558e-6],
    [100, 80, 0.0171083130351331, 0.9828916869648668],
    [100, 100, 0.5132987982791487, 0.48670120172085135],
    [100, 150, 0.9999940754596646, 5.924540335483914e-6],
...
describe('betaPQ', () => {
  const refs: Array<[number, number, number, number, number]> = [
    ...
    [30, 2, 0.5, 1.4901161193847656e-8, 0.9999999850988388],
    [0.5, 2.5, 1e-8, 0.00016976527178252864, 0.9998302347282175],
  ];
...
describe('normalPQ and studentTPQ', () => {
  it('normalPQ is double precision in both tails', () => {
    const refs: Array<[number, number, number]> = [
      [-8, 6.22096057427174e-16, 0.9999999999999993],
      [-1, 0.15865525393145707, 0.8413447460685429],
      [0, 0.5, 0.5],
      [0.5, 0.6914624612740131, 0.3085375387259869],
      [3, 0.9986501019683699, 0.001349898031630093],
      [10, 1, 7.619853024160474e-24],
    ];
  ...
  it('matches reference values for both signs of t, df < 1 through 1e9', () => {
    const refs: Array<[number, number, number, number]> = [
      ...
      [1e6, -5, 2.8669989354453713e-7, 0.9999997133001065],
      [3e6, 2, 0.9772498230593416, 0.022750176940658437],
      [1e9, -4, 3.167124410823391e-5, 0.9999683287558918],
      [4, 1e5, 1, 2.9999999979999997e-20],
      [2.5, -1e3, 2.2747463948307453e-8, 0.9999999772525361],
    ];
...
describe('astronomically large parameters', () => {
  ...
  it("betaPQ stays accurate where ln B would cancel, and past the fraction's reach", () => {
    ...
    for (const [a, b, xs] of [
      [5e9, 5e9 - 1, [0.49999, 0.5, 0.500012]],
      [8e9, 2e9 - 1, [0.79999, 0.8, 0.800009]],
      [3, 1e10 - 4, [5e-11, 2.674e-10, 9e-10]],
      [4e4, 1e10 - 5e4, [3.98e-6, 4e-6, 4.03e-6]],
