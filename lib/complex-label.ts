...
export function complexRootLabel(expr: Expr | undefined, p: number[], env: Record<string, number>): string | undefined {
  ...
  const values = Object.fromEntries(
    [...freeVars(expr)]
      .filter(n => n !== 'w' && Number.isFinite(env[n]))
      .map(n => [n, { kind: 'num' as const, value: env[n] }]),
  );
  const resolved = substVars(expr, values),
    key = exprKey(resolved);
  let labels = cache.get(key);
  if (labels === undefined) {
    labels = complexRootLabels(resolved);
  ...
  return (
    found?.label ??
    (labels.rootOf
      ? `root of ${labels.rootOf} (≈ ${Number(p[0].toPrecision(4))} ${p[1] < 0 ? '−' : '+'} ${Number(Math.abs(p[1]).toPrecision(4))}i)`
      : undefined)
  );
}
