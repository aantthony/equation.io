...
export function fieldEvaluator(comps: Expr[], env: Record<string, number> = {}) {
  if (comps.some(c => exceedsNodes(c, FLOW_NODE_LIMIT)))
    throw new Error('This field is too large to trace (8192 nodes per component).');
...
export function streamline(
  ...
  const direction = (p: number[]) => {
    const v = f(p),
      n = Math.hypot(...v.map((c, k) => c * metric[k]));
  ...
  const side = (sign: number) => {
    ...
    for (let i = 0; i < Math.min(steps, 2048); i++) {
      const a = direction(p);
      if (!a) break;
      const b = direction(at(a, 0.5));
      if (!b) break;
      const c = direction(at(b, 0.5));
      if (!c) break;
      const d = direction(at(c, 1));
      if (!d) break;
...
export function traceField(
  ...
  if (glyphs) {
    ...
    const glyphLen = (span * 0.28) / n;
    for (let i = 0; i < count; i++) {
      const seed = lo.map((v, k) => v + (hi[k] - v) * (((Math.floor(i / n ** k) % n) + 0.5) / n));
      const v = f(seed),
        len = Math.hypot(...v);
    ...
    for (let i = 0; i < FLOW_SEEDS; i++) {
      const seed = lo.map((v, k) => v + (hi[k] - v) * (0.1 + 0.8 * random()));
