...
describe('examples menu', () => {
  ...
  it('never repeats an example', () => {
    ...
    for (const [category, items] of EXAMPLES) {
      for (const [label, text] of items) {
        expect(seen.get(text), `${category} / ${label}`).toBeUndefined();
        seen.set(text, label);
      }
    }
  });
  ...
  // the worst place for one: every row of every example must compile.
  it.each(EXAMPLES.flatMap(([category, items]) =>
    ...
    const errors = analyzeRows(rows).rows.filter(r => r.error).map(r => `${r.text}: ${r.error}`);
