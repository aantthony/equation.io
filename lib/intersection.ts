...
const cross = (a: number[], b: number[]) =>
  a.map((_, k) => a[(k + 1) % 3] * b[(k + 2) % 3] - a[(k + 2) % 3] * b[(k + 1) % 3]);

export function traceIntersection(
  residuals: Expr[],
  lo: number[],
  hi: number[],
  env: Record<string, number> = {},
): number[][][] {
  if (residuals.length !== 2 || lo.length !== 3)
    throw new Error('A space intersection needs two equations in three unknowns.');
  const f = fieldEvaluator(residuals, env);
  let gradient: ReturnType<typeof fieldEvaluator> | undefined;
  try {
    gradient = fieldEvaluator(
      residuals.flatMap(r => ['x', 'y', 'z'].map(v => diff(r, v))),
      env,
    );
  } catch {
    /* finite differences */
  ...
  const correct = (seed: number[], plane?: number[]) => {
    ...
    for (let i = 0; i < 24; i++) {
      ...
      const damping = Math.min(1, (scale * 0.2) / Math.max(error, 1e-30));
  ...
  for (let i = 0; i < 216 && paths.length < INTERSECTION_BRANCHES && budget > 0; i++) {
    const seed = lo.map((v, k) => v + (hi[k] - v) * (((Math.floor(i / 6 ** k) % 6) + 0.43) / 6));
    const root = correct(seed);
    if (!root) continue;
    if (paths.some(path => path.some(p => Math.hypot(...p.map((v, k) => v - root[k])) < step * 1.3))) continue;
    const t0 = tangent(root);
    if (!t0) continue;
    let closed = false;
    const side = (sign: number) => {
      const out: number[][] = [];
      let p = root,
        last = t0.map(v => sign * v);
      for (let n = 0; n < INTERSECTION_STEPS && budget > 0; n++) {
        ...
        if (!next || Math.hypot(...next.map((v, k) => v - predicted[k])) > step * 0.5) break;
        if (n > 8 && Math.hypot(...next.map((v, k) => v - root[k])) < step * 0.75) {
          out.push(root);
          closed = true;
          break;
        }
