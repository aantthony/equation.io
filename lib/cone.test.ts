...
describe('buildUnitCone', () => {
  ...
  it('puts the tip at the origin and the base in z = −1', () => {
    ...
    let sawTip = false, sawBase = false;
  ...
  it('winds side faces so the normal points outward', () => {
    const i0 = mesh.indices[0], i1 = mesh.indices[1], i2 = mesh.indices[2];
    const a = vert(mesh.positions, i0), b = vert(mesh.positions, i1), c = vert(mesh.positions, i2);
  ...
  it('winds the cap so the normal points −z', () => {
    ...
    const i0 = mesh.indices[start], i1 = mesh.indices[start + 1], i2 = mesh.indices[start + 2];
    const n = cross(sub(vert(mesh.positions, i1), vert(mesh.positions, i0)), sub(vert(mesh.positions, i2), vert(mesh.positions, i0)));
