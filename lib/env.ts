...
export type Binding = NumericBinding | Readonly<
  { tag: 'fn'; fn: FnDef }
...
class ReadMap<V> implements ReadonlyMap<string, V> {
  ...
  get(key: string): V | undefined { return this.source().get(key); }
  has(key: string): boolean { return this.source().has(key); }
  entries(): MapIterator<[string, V]> { return this.source().entries(); }
  keys(): MapIterator<string> { return this.source().keys(); }
  values(): MapIterator<V> { return this.source().values(); }
  [Symbol.iterator](): MapIterator<[string, V]> { return this.entries(); }
...
class ReadSet implements ReadonlySet<string> {
  ...
  get size(): number { return this.source().size; }
  has(value: string): boolean { return this.source().has(value); }
  entries(): SetIterator<[string, string]> { return this.source().entries(); }
  keys(): SetIterator<string> { return this.source().keys(); }
  values(): SetIterator<string> { return this.source().values(); }
...
function aliases(name: string, binding: Binding): string[] {
  ...
  if (binding.role === 'state' && binding.init.length !== dim) throw new Error('State derivative and initial dimensions must match.');
...
export class Env {
  ...
  constructor(sequences: Iterable<readonly [string, SeqScan]> = [], readonly sequencePrefix = 'eqioSeq') {
  ...
  drop(name: string): void {
    ...
    if (!entry) return;
    if (entry.kind === 'component') { this.drop(entry.owner); return; }
    for (const alias of aliases(name, entry.binding)) this.owned.delete(alias);
    this.owned.delete(name);
    this.generation++;
  }

  private projection<V>(select: (name: string, value: Binding) => V | undefined, components = false): ReadonlyMap<string, V> {
    let version = -1;
  ...
  readonly states: ReadonlyMap<string, StateDef> = this.projection((_, b) => b.tag === 'scalar' && b.role === 'state' ? { deriv: b.deriv, init: b.init } : undefined, true);
  ...
  readonly lists = this.projection((_, b) => b.tag === 'seq' ? b.value.representation === 'sequence' ? b.value.sequence : b.value.vector : undefined);
  readonly tables = this.projection((_, b) => b.tag === 'table' ? b.table : undefined);
  readonly missingData = this.projection((_, b) => b.tag === 'missing' ? { message: b.message, list: b.list } : b.tag === 'table' ? b.unavailable : undefined);
  readonly rvs = this.projection((_, b) => b.tag === 'rv' ? b.declaration : undefined);
  readonly pointDims: ReadonlyMap<string, number> = this.projection((_, b) => b.tag === 'vector' && b.role !== 'state' ? b.components.length : undefined);
  readonly vecStates: ReadonlyMap<string, number> = this.projection((_, b) => b.tag === 'vector' && b.role === 'state' ? b.deriv.length : undefined);
  readonly points: ReadonlySet<string> = new ReadSet(() => new Set(this.pointDims.keys()));
  readonly scalars: ReadonlyMap<string, ScalarDefinition> = this.projection((_, b) => b.tag === 'scalar' ? b : undefined, true);
}
...
export function lookupValue(env: Env, name: string): Binding | undefined {
  ...
  if (owner?.kind !== 'binding' || owner.binding.tag !== 'vector') throw new Error(`Invalid component owner: ${entry.owner}.`);
  const b = owner.binding, index = entry.index;
...
export function evaluateFrame(env: Env, time: number, stateValues: Record<string, number> = {}): Record<string, number> {
...
export type ValueDefinitions = Pick<Env,
  'consts' | 'fields' | 'states' | 'fns' | 'mats' | 'lists' | 'tables'
  | 'missingData' | 'pointDims' | 'vecStates' | 'points' | 'sequences' | 'sequencePrefix'>;
