...
const outward = (lo: number, hi: number): Interval => Number.isNaN(lo) || Number.isNaN(hi) ? whole() : [nextFloat(lo, false), nextFloat(hi, true)];
...
const idiv = (a: Interval, b: Interval): Interval => b[0] <= 0 && b[1] >= 0 ? whole() : imul(a, outward(1 / b[1], 1 / b[0]));
const excludesZero = (v: Interval) => v[0] > 0 || v[1] < 0;
interface AD { valid: boolean; value: Interval; jac: Interval[] }
...
function intervalAD(e: Expr, names: string[], box: Interval[], env: Record<string, number>): AD {
  ...
  const multiply = (a: AD, b: AD): AD => !a.valid || !b.valid ? bad() : ({ valid: true, value: imul(a.value, b.value), jac: a.jac.map((v, k) => iadd(imul(v, b.value), imul(a.value, b.jac[k]))) });
  const rec = (e: Expr) => intervalAD(e, names, box, env);
  switch (e.kind) {
    case 'num': return Number.isFinite(e.value) ? scalar(point(e.value)) : bad();
    case 'var': {
      const k = names.indexOf(e.name);
      if (k < 0) return Number.isFinite(env[e.name]) ? scalar(point(env[e.name])) : bad();
      const a = scalar(box[k]); a.jac[k] = [1, 1]; return a;
    }
    case 'neg': { const a = rec(e.a); return { valid: a.valid, value: ineg(a.value), jac: a.jac.map(ineg) }; }
    case 'eq': { const a = rec(e.l), b = rec(e.r); return { valid: a.valid && b.valid, value: isub(a.value, b.value), jac: a.jac.map((v, k) => isub(v, b.jac[k])) }; }
    case 'bin': {
      const a = rec(e.a); if (!a.valid) return bad();
      if (e.op === '^') {
        ...
        return { valid: true, value: idiv([1, 1], p.value), jac: p.jac.map(v => ineg(idiv(v, imul(p.value, p.value)))) };
      }
      const b = rec(e.b); if (!b.valid) return bad();
      ...
      return { valid: true, value: idiv(a.value, b.value), jac: a.jac.map((v, k) => idiv(isub(imul(v, b.value), imul(a.value, b.jac[k])), imul(b.value, b.value))) };
    }
    default: return bad();
  }
}
...
export function certifySystem(residuals: Expr[], names: string[], lo: number[], hi: number[],
  env: Record<string, number> = {}, maxBoxes = 2048): Certification {
  const n = names.length;
  const result: Certification = { roots: [], enclosures: [], complete: false, unresolved: 0, visited: 0 };
  if (residuals.length !== n || (n !== 2 && n !== 3) || lo.length !== n || hi.length !== n
    || lo.some((v, k) => !Number.isFinite(v) || !Number.isFinite(hi[k]) || !(v < hi[k]))) throw new Error('Certification needs a finite box and a square 2D or 3D system.');
  if (residuals.some(e => exceedsNodes(e, 2048))) throw new Error('Certification accepts at most 2048 expression nodes per equation.');
  ...
  while (pending.length && result.visited < Math.min(8192, Math.max(1, maxBoxes))) {
    const { box, depth } = pending.pop()!; result.visited++;
    ...
    const cols = Array.from({ length: n }, (_, k) => solveLinear(jm, names.map((_, j) => +(k === j)), n));
    if (f.every(a => a.valid) && atMid.every(a => a.valid) && cols.every(c => c && c.every(Number.isFinite))) {
      ...
      if (excludesZero(determinant(c))) {
        const r = names.map((_, i) => names.map((_, j) => isub(point(+(i === j)), c[i].reduce((sum, v, k) => iadd(sum, imul(v, f[k].jac[j])), [0, 0] as Interval))));
        const kraw = mid.map((m, i) => {
          const center = isub(point(m), c[i].reduce((sum, v, j) => iadd(sum, imul(v, atMid[j].value)), [0, 0] as Interval));
        ...
        const contraction = r.every(row => row.reduce((sum, v) => nextFloat(sum + Math.max(Math.abs(v[0]), Math.abs(v[1])), true), 0) < 1);
        if (contraction && kraw.every((v, i) => v[0] > box[i][0] && v[1] < box[i][1])) {
          const numeric = solveSystem(residuals, names, box.map(v => v[0]), box.map(v => v[1]), { env, seeds: [mid], lattice: false, margin: 0 });
          result.roots.push(numeric[0] ?? kraw.map(([a, b]) => a / 2 + b / 2));
          result.enclosures.push(kraw); continue;
        }
      }
    }
    const widths = box.map(([a, b], k) => (b - a) / (hi[k] - lo[k]));
    const axis = widths.indexOf(Math.max(...widths));
    const split = box[axis][0] + .493 * (box[axis][1] - box[axis][0]);
    if (depth >= 36 || split <= box[axis][0] || split >= box[axis][1]) { result.unresolved++; continue; }
    const a = box.map(v => [...v] as Interval), b = box.map(v => [...v] as Interval);
    a[axis][1] = split; b[axis][0] = split;
