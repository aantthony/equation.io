...
const store =
  (files: Record<string, string> = { 'people.csv': PEOPLE }): TableSource =>
  ({ file }) =>
    files[file] ? parseCsv(files[file]) : null;

/** `null` stands for a device with no local files at all (the worker). */
function build(rows: string[], tables: TableSource | null = store()) {
  return buildDefs(
    rows.map(r => scanDefinition(r)!),
    tables ?? undefined,
  );
}
...
describe('open() rows', () => {
  it('scans as a table definition', () => {
    expect(scanDefinition(`person = open("people.csv", ${HASH})`)).toEqual({
      kind: 'table',
      name: 'person',
      file: 'people.csv',
      hash: HASH,
...
describe('columns as lists', () => {
  ...
  it('broadcasts and zips columns into a scatter', () => {
    ...
    expect((plot as { coords: Float64Array[] }).coords.map(c => [...c])).toEqual([
      [36, 41, 29],
      [1.7, 1.82, 1.65],
    ]);
  });
  ...
  it('names the columns when one is missing', () => {
    expect(() => lowerRow('person.salary', rows)).toThrow(/no column "salary" \(columns: name, age, height\)/);
  });
...
describe('differentiating a list', () => {
  ...
  it('differentiates along x, y, z, which no element can use', () => {
    ...
    const at = (e: Expr, env: Record<string, number>) =>
      (e as Expr & { kind: 'list' }).items.map(it => evaluate(it, env));
    ...
    for (const text of ['total(big.v t)', 'mean(big.v t)', 'min(big.v t)']) {
      ...
      for (let n: Expr | undefined = e; n;) {
        if (n.kind === 'bin') {
          depth++;
          n = n.a;
        } else if (n.kind === 'call' && n.args.length) {
          depth++;
          n = n.args[0];
        } else break;
      }
...
describe('data that is not on this device', () => {
  ...
  it('carries through a filter, so the cut is not reported as broken either', () => {
    const { defs, errors } = build([`person = open("people.csv", ${HASH})`, 'adults = person[person.age >= 18]'], null);
    expect([...errors]).toEqual([]);
  ...
  it('holds on to the name, so a random variable cannot take it on one device', () => {
    ...
    for (const [name, rhs] of [
      ...
      for (const tables of [store(), null]) {
        expect([nameTaken(build(rows, tables).defs, name), name, !!tables]).toEqual([true, name, !!tables]);
      }
    }
    expect(nameTaken(build([`person = open("people.csv", ${HASH})`], null).defs, 'ages')).toBe(false);
  });
  ...
  it('still judges what a filter looks like, so a link is not valid only here', () => {
    ...
    for (const cond of [
      ...
      expect([...build([...rows, `adults = person[${cond}]`], null).errors]).toEqual([['adults', shape]]);
      // …and the device WITH the data agrees, which is the whole point.
      expect([...build([...rows, `adults = person[${cond}]`]).errors]).toEqual([['adults', shape]]);
    }
    expect([...build([...rows, 'adults = person[person.age > t]'], null).errors]).toEqual([
      ['adults', 'A filter cannot depend on t — the list would change length every frame.'],
    ]);
  ...
  it('refuses a filter no list reaches, with the file and without it', () => {
    ...
    for (const idx of ['1 < 2', 'mean(person.age) > 0', 'person.age[1] > 0']) {
      for (const tables of [store(), null]) {
        expect(() => lowerRow(`person.age[${idx}]`, rows, tables), `${idx} ${!!tables}`).toThrow(
          /has to test the list itself/,
        );
      }
    }
    ...
    for (const tables of [store(), null]) {
      expect(() => lowerRow('person.age[domain(person.age) > 0]', rows, tables)).toThrow(
        /Lists cannot appear inside domain/,
      );
    }
    expect(values(lowerRow('person.age[person.age > 30]', rows))).toEqual([36, 41]);
  ...
  it('tells a missing LIST from a missing number', () => {
    ...
    const shape = 'person[…] needs a comparison, like person[person.x > 0].';
    expect(build([...rows, 'avg = mean(person.age)', 'adults = person[avg > 30]'], null).errors.get('adults')).toBe(
      shape,
    );
  });
  ...
  it('points a row that reads a missing file at the file, not at the name', () => {
    ...
    expect(() => listGetter(defs)('person.age')).toThrow(
      /gone\.csv \(a1b2c3d4e5f6\) is not on this device — drop the file here/,
    );
  });
});
...
describe('naming a column, or arithmetic over one', () => {
  ...
  it('reads back as a list wherever it is used', () => {
    ...
    expect(lowerRow('mean(half)', [...rows, 'half = person.age / 2'])).toMatchObject({ kind: 'num' });
  });
  ...
  it('names a scatter of two columns, and it is still a scatter', () => {
    ...
    expect(plot.coords.map(c => [...c])).toEqual([
      [36, 41, 29],
      [1.7, 1.82, 1.65],
    ]);
...
describe('tables and the rest of the definition system', () => {
  it('lets a constant read a reduction of a column', () => {
    const { defs, errors } = build([`person = open("people.csv", ${HASH})`, 'avg = mean(person.age)']);
    expect([...errors]).toEqual([]);
    expect(evaluateFrame(defs, 0).avg).toBeCloseTo(35.3333, 4);
  });

  it('needs the data row above the rows that use it', () => {
    const { errors } = build(['avg = mean(person.age)', `person = open("people.csv", ${HASH})`]);
  ...
  it('plots a big column whole, and expands it only when it must', () => {
    ...
    expect(() => lowerRow('big.v sin(t)', rows, tables)).toThrow(/120000 values; only 100000 can be combined/);
  });
});
