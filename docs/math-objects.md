...
| 4 — space | ~~Lorenz~~ as a 3-component state with `trail` (one trajectory). ~~`revolve()`~~ (plan #7, shipped). ~~complex parametric curves~~ (plan #8, shipped). ~~fields over z + 3D coordinate points~~ (plan #9, shipped). Remaining: 3D named points and geometry (plan #10), 3D vector fields with auto-seeded trajectories (plan #11), 3D chart flows + arrow glyphs (plan #12) | L |
