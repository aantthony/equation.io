...
self.onmessage = (event: MessageEvent<TraceMessage>) => {
  ...
  try {
    ...
    if (input.kind === 'orbit') {
      self.postMessage({ token, result: { pts: traceOrbit(input.orbit!) } });
      return;
    }
    const paths =
      input.kind === 'intersection'
        ? traceIntersection(residuals, lo, hi, env)
        : input.kind === 'field'
          ? traceField(residuals, lo, hi, env, input.glyphs)
          : traceSystem(residuals, dim === 3 ? ['x', 'y', 'z'] : ['x', 'y'], lo, hi, env, 256, angular);
