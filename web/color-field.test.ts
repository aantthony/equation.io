...
const assert = (condition: boolean, why: string) => { if (!condition) throw new Error(why); };
...
if (!gl) status.textContent = 'FAIL: WebGL2 unavailable';
  ...
  const near = (actual: number[], expected: number[], tolerance = 1) =>
    assert(actual.every((x, k) => Math.abs(x - expected[k]) <= tolerance), `${actual} ≠ ${expected}`);
  const check = (label: string, run: () => void) => {
    try { run(); passed++; results.textContent += `PASS ${label}\n`; }
  ...
  const toLab = (rgb: number[]) => {
    const [r,g,b] = rgb.map(x => { const c=x/255; return c<=0.04045 ? c/12.92 : ((c+0.055)/1.055)**2.4; });
