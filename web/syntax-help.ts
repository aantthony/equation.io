...
export function initSyntaxHelp(
  editor: HTMLElement,
  options: {
  ...
  list.id = 'syntax-suggestions';
  list.setAttribute('role', 'listbox');
  list.setAttribute('aria-label', 'Equation suggestions');
  const footer = document.createElement('div');
  footer.className = 'syntax-keys';
  footer.textContent = 'Tab to insert · ↑↓ to choose · Esc to dismiss';
  box.append(hint, list, footer);
  ...
  const accept = (index: number) => {
    ...
    const needsParens =
      !suggestion.insert &&
      suggestion.call &&
      !/^\s*\(/.test(context.text.slice(help.end)) &&
      suggestion.name !== 'int';
  ...
  function refresh() {
    if (composing || document.activeElement !== editor || !getSelection()?.isCollapsed) {
      hide();
      return;
    }
    ...
    if (!help.suggestions.length && !help.hint) {
      hide();
      return;
    }
    hint.textContent = help.hint ?? '';
    hint.hidden = !help.hint;
    ...
    for (const [i, suggestion] of help.suggestions.entries()) {
      ...
      button.type = 'button';
      button.tabIndex = -1;
      button.id = `syntax-option-${i}`;
      button.setAttribute('role', 'option');
      const title = document.createElement('strong');
      title.textContent = suggestion.signature;
